...
//{
//    / <summary>
//    / Ray tracing model for urban propagation
//    / Implements LOS, reflection, diffraction, and scattering mechanisms
//    / 
//    / Based on:
//    / - Rappaport "Wireless Communications" (2002) - Ch 4-5
//    / - Saunders & Aragon-Zavala "Antennas and Propagation" (2007) - Ch 3
//    / - ITU-R P.526 (diffraction)
//    / - ITU-R P.2040 (material properties)
//    / 
//    / Note: Uses coherent combination for narrowband signals (BW << fc)
//    / For wideband, each subcarrier would need separate tracing
//    / </summary>
...
//        [Range(0f, 1f)] public float defaultScatterAlbedo = 0.2f;
//        [Range(0f, 8f)] public int scatterLobeExponent = 2;
...
//            public float ExtraPhaseRad; // for reflection/diffraction phase shifts
//        }
...
//                if (maxReflections > 0)
//                    TrySingleBounceReflections();

//                if (maxDiffractions > 0)
//                    TrySingleEdgeDiffractions();

//                if (enableDiffuseScattering)
//                    TrySingleBounceScattering();

//                float result;
//                if (paths.Count > 0)
//                {
//                     combine paths coherently (phasor addition)
//                    result = CombinePathsCoherent(paths);
//                }
//                else
//                {
...
//         Single bounce reflections using image method
//        private void TrySingleBounceReflections()
//        {
//            var tx = context.TransmitterPosition;
//            var rx = context.ReceiverPosition;

//             search box around TX-RX link
//            var mid = 0.5f * (tx + rx);
...
//                     check if path from reflection point to RX is clear
...
//                        float reflCoefficient = BuildingMaterial.GetReflectionCoefficient(
//                            context.FrequencyMHz / 1000f, material);
...
//                     calculate phase from Fresnel equations
//                    float reflPhase = CalculateReflectionPhase(epsilonR, sigma,
//                        context.FrequencyMHz, incidenceAngleRad);
...
//        private void TrySingleEdgeDiffractions()
//        {
//            var tx = context.TransmitterPosition;
//            var rx = context.ReceiverPosition;
...
//                 snap to actual roof surface
//                if (!SnapToColliderSurface(p + Vector3.up * 1.0f, Vector3.up, 5f,
//                        context.BuildingLayer, e.building, out var pRoof, out var nRoof))
//                    continue;
...
//                    LossDb = loss,
//                    DistanceMeters = totalDist,
//                    ExtraPhaseRad = -Mathf.PI * 0.25f // -pi/4 for diffraction
//                });
...
//        private void TrySingleBounceScattering()
//        {
//            if (!enableDiffuseScattering)
//                return;
...
//                    {
//                        if (!SnapToColliderSurface(p0 + wall.Normal * 0.5f, wall.Normal, 2.0f,
//                                context.BuildingLayer, col, out var pHit, out var nHit))
//                            continue;
...
//                         roughness factor from Rayleigh criterion
//                        float k0 = 2f * Mathf.PI / Mathf.Max(context.WavelengthMeters, EPS);
//                        float sinPsi = Mathf.Sqrt(Mathf.Max(0f, 1f - cos_i * cos_i));
...
//                        float materialLoss = -10f * Mathf.Log10(Mathf.Max(S, 0.01f));
//                        float directionalLoss = -10f * Mathf.Log10(Mathf.Max(diffuseMag, 1e-4f));

//                        float loss = fspl + scatterBaseLossDb + materialLoss + directionalLoss;

//                        bestLossDb = Mathf.Min(bestLossDb, loss);

//                         Note: scattering is incoherent so we don't add to paths list
//                         just track it for visualization and best loss
...
//                            phaseRadians = 0f,
//                            extraPhaseRad = 0f,
//                            label = $"Diffuse via {col.name} (loss {loss:F1} dB, incoherent)"
//                        };
...
//         Image method for finding specular reflection point
...
//             mirror RX across the wall plane
//            var d = Vector3.Dot(rx - q, n);
//            var rxImage = rx - 2f * d * n;

//             find where TX->RX_image intersects the plane
//            var v = rxImage - tx;
...
//        private static IEnumerable<Vector3> SampleAroundCenter(Vector3 center, Bounds b, Vector3 n,
//            int count, float radiusFrac)
//        {
...
//                 skip buildings that are way off to the side
...
//                 add vertical edges if building is close to path
...
//        private static Vector3 ClosestPointOnEdgeToLine(Vector3 tx, Vector3 rx,
...
//        private static bool EdgeObstructsPath(Vector3 tx, Vector3 rx, Vector3 edgePoint,
//            Vector3 edgeStart, Vector3 edgeEnd)
//        {
//            var lineDir = (rx - tx).normalized;
...
//        / <summary>
//        / Coherent combination of paths using phasor addition
//        / This is the correct way to combine narrowband multipath components
//        / since phases from geometry are deterministic (not random)
//        / </summary>
//        private float CombinePathsCoherent(IList<PathContribution> paths)
//        {
//            if (paths.Count == 0)
...
//         Knife edge diffraction loss from ITU-R P.526
...
//        / <summary>
//        / Calculate reflection phase from Fresnel equations
//        / Based on Balanis "Advanced Engineering Electromagnetics" Ch 5
//        / Uses perpendicular polarization assumption
//        / </summary>
//        private static float CalculateReflectionPhase(float relativePermittivity, float conductivity,
//            float frequencyMHz, float incidenceAngleRad)
//        {
...
//             calculate sqrt term in Fresnel equation
...
//             reflection coefficient for perp polarization
...
//            {
//                return Mathf.PI; // fallback for grazing
//            }
