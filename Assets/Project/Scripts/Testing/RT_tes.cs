...
//        public PropagationContext context;
...
//        {
//            buildingLayer = context.BuildingLayer;
//            context = ctx;
...
//            try
//            {
//                TryDirect(context, ref bestLossDb, paths);
//                if (maxReflections > 0) TrySingleBounceReflections(context, ref bestLossDb, paths);
//                if (maxDiffractions > 0) TrySingleEdgeDiffractions(context, ref bestLossDb, paths);
//                //if (enableDiffuseScattering) TrySingleBounceScattering(context, ref bestLossDb, paths);
...
//        // 1) Direct
//        // -----------------------------
//        private void TryDirect(PropagationContext context, ref float bestLossDb, List<PathContribution> paths)
//        {
...
//            if (Physics.Raycast(tx, dir, out var hit, dist ))
//            {
...
//        // -----------------------------
//        private void TrySingleBounceReflections(PropagationContext context, ref float bestLossDb, List<PathContribution> paths)
//        {
//            var tx = context.TransmitterPosition;
//            var rx = context.ReceiverPosition;
...
//            var colliders = Physics.OverlapBox(mid, halfExtents, Quaternion.identity );
...
//                    // LoS checks
//                    if (Physics.Raycast(tx, (p - tx).normalized, out var hit1, d1))
//                    {
...
//                        continue;
//                    }
//                    if (Physics.Raycast(rx, (p - rx).normalized, out var hit2, d2))
//                    {
//                        if (enableRayVisualization && showReflectionRays && showBlockedPaths && Visualizer != null)
...
//                        gamma = Mathf.Clamp01(BuildingMaterial.GetReflectionCoefficient(context.FrequencyMHz, material));  // 0..1  (from your ScriptableObject)  :contentReference[oaicite:4]{index=4}
//                    }
...
//                    var loss = FSPL(context.FrequencyMHz, d1) + FSPL(context.FrequencyMHz, d2) + reflLossDb;
...
//        // -----------------------------
//        private void TrySingleEdgeDiffractions(PropagationContext context, ref float bestLossDb, List<PathContribution> paths)
//        {
//            var tx = context.TransmitterPosition;
//            var rx = context.ReceiverPosition;
...
//                // Knife-edge loss
//                var lambda = context.WavelengthMeters; // c/f in meters
//                var v = FresnelParameterV(tx, rx, p, lambda);
//                var diffLossDb = KnifeEdgeDiffractionLossDb(v);

//                var loss = FSPL(context.FrequencyMHz, d1) + FSPL(context.FrequencyMHz, d2) + diffLossDb;
//                bestLossDb = Mathf.Min(bestLossDb, loss);
//                paths.Add(new PathContribution
//                {
//                    LossDb = loss,
...
//        //private void TrySingleBounceScattering(PropagationContext context, ref float bestLossDb, List<PathContribution> paths)
//        //{
...
//        //    var cols = Physics.OverlapBox(mid, halfExtents, Quaternion.identity);
...
//        //                if (Occluded(tx, p, col)) continue;
//        //                if (Occluded(p, rx, col)) continue;

//        //                // material params
//        //                var bld = col.GetComponent<RFSimulation.Environment.Building>();
//        //                float S = bld && bld.material ? Mathf.Clamp01(bld.material.scatterAlbedo) : defaultScatterAlbedo;
//        //                var material = bld.material;
//        //                float rhoSmooth = bld && bld.material ? Mathf.Clamp01(bld.material.GetReflectionCoefficient(context.FrequencyMHz, material)) : 0.3f;
...
//        //                float loss = FSPL(context.FrequencyMHz, d1) + FSPL(context.FrequencyMHz, d2)
...
//        private bool Occluded(Vector3 a, Vector3 b, Collider ignore = null)
//        {
...
//            if (Physics.Raycast(a2, dir, out var hit, maxDist, buildingLayer))
//            {
